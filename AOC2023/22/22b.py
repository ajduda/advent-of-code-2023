...
supportedByBase = supportedBy

print('figuring out the answer')
for k in brickLocations:
    print(k)
    #Deep copy the origional supportedByBase dictionary
    supportedBy = {}
    for k2 in supportedByBase:
        supportedBy[k2] = supportedByBase[k2].copy()
    #Falling and desintegrating are basically the same thing, right?
    fallen = {k}
    toProcess = {k}
    otherFallen = -1  # count the other bricks that fall. Done by counting all bricks that fall and ignoring the first one by setting this number to -1
    while len(toProcess) > 0:
        n = toProcess.pop()
        otherFallen += 1
        if n not in supporting:
            continue
        for unsupported in supporting[n]:
            supportedBy[unsupported].remove(n)
            if len(supportedBy[unsupported]) == 0:
                toProcess.add(unsupported)
    ans += otherFallen
